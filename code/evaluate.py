...
def run_benchmark(entry: MaxXorSat, qaoa_reps=1, grover_iterations=None):
    ...
    try:
        ...
        if len(sol_grover) == entry.m:
            ...
            results["grover_ratio"] = util_grover / util_exact if util_exact > 0 else 1.0
...
def plot_results(df):
    ...
    df_time = df[['n', 'exact_time', 'qaoa_time', 'grover_time']].melt('n', var_name='Algo', value_name='Temps (s)')
    sns.lineplot(data=df_time, x='n', y='Temps (s)', hue='Algo', marker='o', ax=axes[0, 0])
    ...
    df_quality = df[['n', 'qaoa_ratio', 'grover_ratio']].melt('n', var_name='Algo', value_name='Ratio Utilité')
    ...
    df_depth = df[['n', 'qaoa_depth', 'grover_depth']].melt('n', var_name='Algo', value_name='Profondeur')
    sns.lineplot(data=df_depth, x='n', y='Profondeur', hue='Algo', marker='o', ax=axes[1, 0])
    axes[1, 0].set_title("Profondeur du circuit vs n")
    
    # 4. Complexité : Nombre de portes
    df_gates = df[['n', 'qaoa_gates', 'grover_gates']].melt('n', var_name='Algo', value_name='Nombre de portes')
    sns.lineplot(data=df_gates, x='n', y='Nombre de portes', hue='Algo', marker='o', ax=axes[1, 1])
...
if __name__ == "__main__":
    ...
    for rep in qaoa_reps_list:
        ...
        for i, instance in enumerate(instances):
            print(f"Traitement instance {i+1}/{num_instances} (n={instance.n}, m={instance.m})...")
    ...
    # Export CSV 
    output_file = "resultats_benchmark.csv"
